(function(){
    ...
    var funcManageProduct = function($scope,$stateParams,$state,commonAppService,ManageProduct,ProductInfo,$ionicPopup,$cordovaCamera,$cordovaDevice, $cordovaFile, $ionicPlatform, $ionicActionSheet, ImageService,ApiEndpoint,$timeout,base64){
        ...
        $scope.subCatgCheckboxes={sub_category_id:[]};
        $scope.subCatgChecked=[];
        ...
        $scope.fetchMasterCategoryList = function(){
            ...
            ManageProduct.fetchMasterCategory().then(function(result){
                console.log("Master category in controller from DB is"+JSON.stringify(result));
                  ...
                  /* console.log("key is "+key);
                   console.log("value is "+JSON.stringify(value));*/
                    /*angular.forEach(angular.fromJson(value.sub_category_list),function(val1,key1){
                        if(val1.exist){*/
                            //console.log("subg cat id checked is "+val1.sub_category_id)
                            ...
                            //console.log("subg cat id in "+JSON.stringify($scope.subCatgChecked));
                            /*$scope.subCatgChecked.push({
                                 sub_category_id: val1.sub_category_id
                            });*/
                            /*console.log("subg cat id in "+JSON.stringify($scope.subCatgChecked));
                        }
        ...
        $scope.checkBoxCatgClicked = function(list){
             
             /*var idx = $scope.masterCategory.indexOf(mainIndex);
             console.log(" Main Index and passed value "+JSON.stringify(mainIndex));*/
             console.log(" sub catg clicked  list "+JSON.stringify(list));
             list.exist = !list.exist;
             /*$scope.subCatgChecked.push({
                 sub_category_id: subCatgId
            });
             console.log("subg cat id in  subcatg checked list"+JSON.stringify($scope.subCatgChecked));
             */
         }
        ...
        $scope.alertProductCategory_save = function() {
               var alertPopup = $ionicPopup.alert({
                 title: 'Save category successfull !!',
                 template: ''
               });
        ...
        $scope.alertProductCategory_saveError = function() {
               var alertPopup = $ionicPopup.alert({
                 title: 'Some issues while saving category !!',
        ...
        $scope.enableCatgAndSubCatgToSeller = function(masterCategory) {
            var list = "";
            /*var urlValues = "";
           ...
           angular.forEach(masterCategory,function(value,key){
                    console.log("Final List of catg id is"+JSON.stringify(masterCategory));
                    angular.forEach(angular.fromJson(value.sub_category_list),function(val1,key1){
                    if(val1.exist){
                            console.log("Final List of sub catg id is"+val1.sub_category_id);
                            list += "sub_category_list[]="+val1.sub_category_id+"&";
                    }
                });
                    
            });
            /*angular.forEach($scope.subCatgChecked,function(val,key){
               console.log("list inside subCatgChecked- valis "+val);
               console.log("list inside subCatgChecked-key is "+key);
               list += "sub_category_list[]="+val+"&";
            });*/
            ...
            ManageProduct.enableCatgAndSubCatg(list).then(function(results){
                ...
                if (results.status == 200 && results.data ) {
                    console.log(" result.status and data "+JSON.stringify(results.data));
                    
                    $scope.alertProductCategory_save();
                }else{
                    $scope.alertProductCategory_saveError();
                }
            }); 
        };
     ...
     var funcEditProduct = function($scope,$stateParams,$state,commonAppService,ManageProduct,ProductInfo,$ionicPopup,ApiEndpoint,$ionicActionSheet,base64,$timeout,ImageService){
        ...
        $scope.editProductDetails = function(){
            ...
            ManageProduct.getProductDetailsForEdit(product_id).then(function(results){
                ...
                console.log("krishna price"+JSON.stringify(prices));
                if(prices.length < 4 ) {
